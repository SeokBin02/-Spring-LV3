...
public class PostService extends SuperService {
    ...
    //    2. 게시글 작성 API
    @Transactional
    public PostResponseDto createPost(PostCURequestDto requestDto, @AuthenticationPrincipal UserDetailsImpl userDetails) {
//        User user = getUserInfoFromToken(request);                  // 토큰 유효성 검사를 거친 후 user 정보를 가져옴
    ...
    //    4. 선택한 게시글 수정 API
    @Transactional
    public PostResponseDto updatePost(Long id, PostCURequestDto requestDto, @AuthenticationPrincipal UserDetailsImpl userDetails) {
//        User user = getUserInfoFromToken(request);      // 토큰 유효성 검사를 거친 후 user 정보를 가져옴
        Post post = validateIsPosts(id);                // 조회하려는 게시판의 존재 유무 체크
        isCheckAuthorFromPosts(userDetails.getUser(), post);             // 해당 게시판의 수정 권한 유무 체크
    ...
    //    5. 선택한 게시글 삭제 API
    @Transactional
    public PostDeleteResponseDto deletePost(Long id, @AuthenticationPrincipal UserDetailsImpl userDetails) {
//        User user = getUserInfoFromToken(request);      // 토큰 유효성 검사를 거친 후 user 정보를 가져옴
        Post post = validateIsPosts(id);                // 조회하려는 게시판의 존재 유무 체크
        isCheckAuthorFromPosts(userDetails.getUser(), post);             // 해당 게시판의 삭제 권한 유무 체크
